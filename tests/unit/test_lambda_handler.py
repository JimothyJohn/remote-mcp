...
import time
import typing
from remote_mcp.server.mcp_lambda_handler import MCPLambdaHandler, SessionData
from remote_mcp.server.mcp_lambda_handler.session import (
    DynamoDBSessionStore,
    NoOpSessionStore,
)
from remote_mcp.server.mcp_lambda_handler.types import (
    Capabilities,
    ErrorContent,
    FileResource,
    ImageContent,
...
def test_get_set_update_session(monkeypatch):
    ...
    from remote_mcp.server.mcp_lambda_handler import current_session_id
...
def test_create_error_and_success_response():
    ...
    ok = handler._create_success_response(
        {"foo": "bar"}, request_id="abc", session_id="sid"
    )
    if "result" in ok:
...
def test_handle_request_session_required():
    ...
    with patch("boto3.resource") as mock_resource:
        ...
        handler = MCPLambdaHandler(
            "test-server", session_store=DynamoDBSessionStore("tbl")
        )
...
def test_tool_decorator_enum_type():
    ...
    assert set(schema["inputSchema"]["properties"]["color"]["enum"]) == {
        "red",
        "blue",
        "green",
    }
...
def test_tool_decorator_dictionary_type_hints():
    ...
    assert (
        schema["inputSchema"]["properties"]["simple_dict"]["additionalProperties"][
            "type"
        ]
        == "integer"
    )
...
def test_tool_decorator_recursive_dictionary_type_hints():
    ...
    value_schema = schema["inputSchema"]["properties"]["nested_dict"][
...
def test_handle_image_byte_streams():
    ...
    for format_name, format_data in image_data.items():
        ...
        if isinstance(resp, dict) and "body" in resp:
            ...
            assert isinstance(body["result"], dict), (
                f"Result not a dict for {format_name}"
            )
            assert "content" in body["result"], (
                f"No content in result for {format_name}"
            )
            assert isinstance(body["result"]["content"], list), (
            ...
            assert content["mimeType"] == format_data["mime"], (
                f"Wrong MIME type for {format_name}"
            )
            ...
            assert decoded_bytes == format_data["bytes"], (
                f"Data mismatch for {format_name}"
            )
        else:
...
def test_handle_request_malformed_jsonrpc():
    ...
    assert (
        resp["statusCode"] == 400
        or resp["statusCode"] == 500
        or resp["statusCode"] == 400
    )
...
def test_resource_content_model_dump():
    ...
    content = ResourceContent(
        uri="test://resource", mimeType="text/plain", text="Hello World"
    )
    ...
    content_blob = ResourceContent(
        uri="test://resource2", mimeType="image/png", blob="base64data"
    )
...
def test_file_resource_json_file():
    ...
    try:
        resource = FileResource(
            uri="file://test.json", path=temp_path, name="Test JSON"
        )
...
def test_file_resource_yaml_file():
    ...
    try:
        resource = FileResource(
            uri="file://test.yaml", path=temp_path, name="Test YAML"
        )
...
def test_multiple_resources_same_handler():
    ...
    try:
        # Add file resource
        file_resource = FileResource(
            uri="file://test3", path=temp_path, name="File Test 3"
        )
        handler.add_resource(file_resource)
